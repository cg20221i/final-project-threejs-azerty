...
export function Frames({ images, q = new THREE.Quaternion(), p = new THREE.Vector3() }) {
  ...
  useFrame((state, dt) => {
    easing.damp3(state.camera.position, p, 0.4, dt, 5)
    easing.dampQ(state.camera.quaternion, q, 0.4, dt, 5)
  })
  return (
    <group
      ref={ref}
  ...
  return (
    <group {...props}>
      <mesh
        ...
        <Image raycast={() => null} ref={image} position={[0, 0, 0.7]} url={url} scale={2} />
      </mesh>
      <mesh
        onClick={() => {
          router.push(linkTo)
